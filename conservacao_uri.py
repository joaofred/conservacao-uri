...
'''
def checarpearson_paralelo(par):
    lista = [[rbc_gel, rbc_amb], [muc_amb, muc_gel], [caoxd_amb, caoxd_gel], [hya_amb, hya_gel], [bac_amb, bac_gel], 
         [pat_amb, pat_gel], [wbc_amb, wbc_gel], [epi_amb, epi_gel], [tri_amb, tri_gel], [uri_amb, uri_gel], 
         [yea_amb, yea_gel], [amo_amb, amo_gel]]
    relatorio = []    
    for ex in lista:
        ...
        for i in ["H4", "H8", "H12", "H24"]:
            p = ""        
            resultado = pearsonr(np.array(exame1[i].map(float)), np.array(exame2[i].map(float)))
            relatorio.append(resultado)
            if resultado[1] < 0.05: p = "***"        
            print("Entre",i,"e", i,":", resultado[0], "(Pearson ρ), e ", resultado[1],"(valor de p)", p)
    print(___l)
    return(relatorio)
...
def checarlinreg1(t="H24"):
    ...
    print(__l)
    print()
    print("Regressão Linear",t,"(GEL) x",t,"(AMB) para os diversos exames")    
    print()
    print(__l)
    for i in lista:
        p, p1 = i[0], ""
        c1 = i[0][t].map(float)
        c2 = i[1][t].map(float)
        lr = sp.stats.linregress([c1,c2])
        if lr[3] < 0.05: p1 = "***"
        print(list(Series(p["EXAME"]))[0], ":", )
        print("   ",lr[0],"(interc); ", lr[2],"(valor de r); ", "\n", "  ", lr[1],"(std-err); ",lr[3],"(valor de p)", p1)
    print(__l)
    #slope intercept r p std

def checarlinreg2(t="H24"):
    lista = [[rbc_amb, rbc_gel], [muc_amb, muc_gel], [caoxd_amb, caoxd_gel], [hya_amb, hya_gel], [bac_amb, bac_gel], 
     [pat_amb, pat_gel], [wbc_amb, wbc_gel], [epi_amb, epi_gel], [tri_amb, tri_gel], [uri_amb, uri_gel], 
     [yea_amb, yea_gel], [amo_amb, amo_gel]]
    print()
    print(__l)
    print()
    print("Regressão Linear H0 x",t,"para os diversos exames")    
    print()
    print(__l)
    for i in lista:        
        p, p1, p2 = i[0], "", ""
        c1, c2 = i[0]["H0"].map(float), i[0][t].map(float)
        d1, d2 = i[1]["H0"].map(float), i[1][t].map(float)
        lr1 = sp.stats.linregress([c1,c2])
        lr2 = sp.stats.linregress([d1,d2])
        if lr1[3] < 0.05: p1 = "***"
        if lr2[3] < 0.05: p1 = "***"
        print(list(Series(p["EXAME"]))[0], "AMB:", )
        print("   ",lr1[0],"(interc); ", lr1[2],"(valor de r); ", "\n", "  ", 
              lr1[1],"(std-err); ",lr1[3],"(valor de p)", p1)
        print("   ", "GEL:", )
        print("   ",lr2[0],"(interc); ", lr2[2],"(valor de r); ", "\n", "  ", 
              lr2[1],"(std-err); ",lr2[3],"(valor de p)", p1)
        print()
    print(__l)
...
>>>>>>> fe49e2b8f8cdcfaa437622c6f6b7314efe00257b
'''
